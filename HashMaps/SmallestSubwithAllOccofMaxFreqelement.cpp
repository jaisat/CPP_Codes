...
void c_p_c() {
	...
	for(int end = 0; end < n; end++){
		freq[arr[end]]++;
		if(start.find(arr[end]) == start.end()){
			start[arr[end]] = end;
		}
		if(freq[arr[end]] > highestFreq){
			highestFreq = freq[arr[end]];
			num = arr[end];
			ans = end - start[arr[end]] + 1;
			e = end;
			s = start[arr[end]];
		}else if(freq[arr[end]] == highestFreq){
			if(end - start[arr[end]] + 1 < ans){
				s = start[arr[end]];
				e = end;
				ans = end - start[arr[end]] + 1;
				num = arr[end];
